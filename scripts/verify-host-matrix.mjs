...
for (const entry of await readdir(join(root, "packages"), {
  ...
  if (Array.isArray(manifest.pi?.extensions)) packageNames.push(manifest.name);
}
packageNames.sort();
  ...
  const consume = (chunk) => {
    ...
    for (const line of lines) {
      try {
        ...
        // Host startup banners are ignored; only JSONL frames carry results.
      }
    }
  };
...
console.log(`verify-host-matrix: ${host} loaded ${packageNames.join(", ")}`);
