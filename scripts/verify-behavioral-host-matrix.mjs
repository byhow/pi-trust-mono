...
try {
  ...
  const commandType =
    host === "omp" ? "get_available_commands" : "get_commands";
  const hostArgs =
    host === "omp"
  ...
  const invokeCommandExpectingNotification = (message, expectedMessage) => {
    ...
    const timeout = setTimeout(
      () =>
        finish(
          new Error(`${host}: handler invocation timed out for ${message}`),
        ),
      20_000,
    );
  ...
  const commandCanaries = new Map([
    ...
    ["mcp-vet", ["/mcp-vet", "Usage: /mcp-vet <descriptor-json>"]],
    [
      "model-picker",
      [
        "/model-picker",
        "Model selection failed with code selector-invalid. Install the reviewed model-picker release that provides model-picker.selection v1 and set PI_MODEL_PICKER_BIN to its absolute launcher path.",
      ],
    ],
    ["permit", ["/permit", "Usage: /permit <tool-name>"]],
    [
      "recall",
      ["/recall fixture", "The configured warm-memory archive path is unsafe."],
    ],
  ]);
  for (const expected of expectedCommands) {
    const canary = commandCanaries.get(expected);
    if (!canary)
      throw new Error(`${host}: command has no behavioral canary: ${expected}`);
    await invokeCommandExpectingNotification(...canary);
  }
  console.log(
    `verify-behavioral-host-matrix: ${host} loaded ${extensionPackages.join(", ")}, registered ${[...expectedCommands].sort().join(", ")}, and invoked every command handler`,
  );
} finally {
