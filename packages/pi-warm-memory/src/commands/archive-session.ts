...
const getSessionId = (ctx: HookCommandContext): string => {
  ...
  const sessionId = filename.includes("_") ? filename.split("_").pop()! : filename
...
const getGitContext = async (api: CustomCommandAPI, cwd: string) => {
  const run = async (cmd: string, args: string[]) => {
    try {
      ...
      return r.code === 0 && !r.killed ? r.stdout.trim() || undefined : undefined
    } catch {
      return undefined
    }
  }
  ...
  const branch = repoRoot ? await run("git", ["rev-parse", "--abbrev-ref", "HEAD"]) : undefined
  const shortSha = repoRoot ? await run("git", ["rev-parse", "--short", "HEAD"]) : undefined
...
const createCommand = (api: CustomCommandAPI): CustomCommand => ({
  ...
  async execute(args, ctx) {
    ...
    const instruction = (packetKind === "checkpoint" ? args.slice(1) : args).join(" ").trim()
    ...
    const templateFile = packetKind === "handoff" ? "handoff-packet.md" : "checkpoint-packet.md"
    const template = await readFile(resolve(templatesDir, templateFile), "utf8")
    ...
    const packetDirRel = relative(ctx.cwd, resolve(historyDir, "packets", year, month))
